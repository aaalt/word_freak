...
UJ txt_get_word(STR dir, STR source, UJ max_d, UJ max_s, UJ ptr)
{
	LOG("txt_get_word");
	UJ i, j;
...
UJ txt_proc_buf(STR buf, V* hsh, V* stop, I len, WORD_ADD fn, I param, sz el)
{
	...
	W(PTR < len - 1) {
		...
		GET_TXT:
		var = txt_get_word((STR)NIL, buf, NIL, len, PTR);	
...
UJ txt_process(FILE* f, V* struct_1, V* struct_2, WORD_ADD fn, sz el)
{
	...
	i = txt_proc_buf(TEXT_BUF, struct_1, struct_2, len + 1, fn, feof(f), el);
	P(i == NIL, NIL);
	cnt += i;

	// T(INFO, "\t[~]\tmfread and txt_proc for %lu words\t\t\t%lums\t%lums", i, m, t);
	if (!feof(f)) 
		goto LOOP;

	set_glb(&WORD_LEN, 0, &SWIPE_NEXT_VALS, 0, NULL, 0);
